import type { PrismaClient, FinancialProfile } from "@maplewealth/db";
import { evaluateRules, type RuleResult } from "./rules";
import { getContributionRoom } from "./contributions";

type ContributionRoom = Awaited<ReturnType<typeof getContributionRoom>>;

export async function getMonthlyReport(prisma: PrismaClient, userId: string, monthStr: string) {
  return generateReportData(prisma, userId, monthStr);
}

export async function generateReportData(prisma: PrismaClient, userId: string, monthStr: string) {
  ...
  const warnings = await evaluateRules(prisma, userId);
  const room = await getContributionRoom(prisma, userId);

  // Deterministic (non-AI) template filled with the user's actual computed numbers —
  // not a generative-AI call.
  const reportText = compileSummary({
    monthStr,
    income,
    expenses,
    savings,
...
function compileSummary(data: {
  ...
  const overcontributed =
    data.room.tfsa.overLimit || data.room.fhsa.overLimit || data.room.rrsp.overLimit;
