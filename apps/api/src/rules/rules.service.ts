  ...
  async evaluateRules(userId: string): Promise<RuleResult[]> {
    ...
    if (efBalance < 1000) {
      ...
      results.push({
        ...
        recommended_action:
          'You can comfortably focus on TFSA and registered investments.',
        source_rule: 'Emergency Fund Ideal Target',
      });
    }
    ...
    const rrspTotal = rrspContributions.reduce(
    ...
    if (room.fhsa.overLimit) {
      results.push({
        ...
        recommended_action:
          'Withdraw the excess contributions to avoid the 1% monthly penalty from the CRA.',
